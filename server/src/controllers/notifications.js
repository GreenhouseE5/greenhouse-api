...
const count = require('../db/controllers/count')
...
const addNotification = async ({ body }, res, next) => {
  try {
    ...
    if (client) {
      global.io.to(client.socket).emit('notification', JSON.stringify(body))
    }
...
const getNotifications = async ({ query }, res, next) => {
  try {
    ...
    const notificationsTotal = await count(models.NOTIFICATION, filters)
    res.status(200).json({ results: ntfcs, total: notificationsTotal, offset })
  } catch (error) {
    next(error)
  }
}
...
const getOneNotification = async ({ params }, res, next) => {
  try {
    const notification = await findOne(models.NOTIFICATION, { ...params, isActive: true })
    if (!notification) throw new HttpError(400, 'No se encontró la notifcación')
...
const deleteNotification = async ({ params }, res, next) => {
  try {
    ...
    if (!resp.nModified) {
      throw new HttpError(400, `Notification ${params._id} not exist`)
    }
